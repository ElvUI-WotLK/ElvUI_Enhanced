...
local LOS = E:NewModule("Enhanced_LossOfControl", "AceEvent-3.0")
...
function LOS:UNIT_AURA()
	...
	for i = 1, 40 do
		...
		if(E.db.enhanced.lossofcontrol[abilities[name]] and expirationTime > maxExpirationTime) then
...
function LOS:Initialize()
	if not E.db.enhanced.lossofcontrol.enable then return end

	self.frame = CreateFrame("Frame", "ElvUI_lossofcontrolFrame", UIParent)
	self.frame:Point("CENTER", 0, 0)
	self.frame:Size(54)
	self.frame:SetTemplate()
	self.frame:Hide()

	for name, v in pairs(G.lossofcontrol) do
		if(name) then
			abilities[name] = v
		end
	end
