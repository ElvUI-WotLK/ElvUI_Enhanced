					...
					model = {
						...
						args = {
							enable = {
								...
								type = "toggle",
								name = L["Enable"],
								get = function(info) return E.db.enhanced.model.enable end,
								set = function(info, value) E.db.enhanced.model.enable = value E:StaticPopup_Show("PRIVATE_RL") end
							}
						}
					}
				}
...
local function TimerTrackerOptions()
	local config = {
		...
		get = function(info) return E.db.enhanced.timer[ info[#info] ] end,
		set = function(info, value) E.db.enhanced.timer[ info[#info] ] = value E:GetModule("Enhanced_TimerTracker"):UpdateAllIconsTimers() end,
		args = {
			enable = {
				order = 1,
				type = "toggle",
				name = L["Enable"],
				get = function(info) return E.db.enhanced.timer.enable end,
				set = function(info, value) E.db.enhanced.timer.enable = value E:StaticPopup_Show("PRIVATE_RL") end
			},
			dbm = {
				order = 2,
				type = "toggle",
				name = L["Hook DBM"],
				get = function(info) return E.db.enhanced.timer.dbm end,
				set = function(info, value) E.db.enhanced.timer.dbm = value E:GetModule("Enhanced_TimerTracker"):HookDBM() end,
				disabled = function() return not E.db.enhanced.timer.enable end
			}
		}
	}
	return config
